"use client";

import { X, CheckCircle2 } from "lucide-react";
import { Button } from "@/components/ui/button";

interface ConcluirAgendamentoModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: () => void;
}

export function AgendamentoModalConcluir({
  isOpen,
  onClose,
  onConfirm,
}: ConcluirAgendamentoModalProps) {
  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl p-8 w-90 md:w-130 flex flex-col items-center shadow-xl zoom-in-95">
        <h2 className="text-xl font-bold text-[#344054] mb-2 text-center">
          Concluir agendamento?
        </h2>

        <p className="text-[#344054] text-sm mb-8 text-center">
          Deseja realmente marcar este agendamento como concluído? Esta operação atualizará o status do atendimento.
        </p>

        <div className="flex flex-col md:flex-row gap-3 w-full">
          <Button
            variant="outline"
            onClick={onClose}
            className="w-full md:flex-1 rounded-full border-[#165BAA] text-[#165BAA] hover:bg-blue-50 h-11 cursor-pointer"
          >
            <X size={18} className="mr-2" />
            Voltar
          </Button>

          <Button
            onClick={onConfirm}
            className="w-full md:flex-1 rounded-full bg-[#165BAA] hover:bg-[#13447D] text-white h-11 cursor-pointer"
          >
            <CheckCircle2 size={18} className="mr-2" />
            Concluir
          </Button>
        </div>
      </div>
    </div>
  );
}