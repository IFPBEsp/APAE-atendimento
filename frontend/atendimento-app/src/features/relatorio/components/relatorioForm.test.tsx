...
describe("RelatorioForm - Validações e Cénarios Negativos", () => {
    ...
    it("deve bloquear a submissão com data futura ou fora do limite institucional (30 anos)", async () => {
        ...
        const btnSubmit = screen.getByRole("button", { name: /Adicionar Relatório/i });

        const dataFutura = new Date();
        dataFutura.setFullYear(dataFutura.getFullYear() + 1);

        fireEvent.change(inputData, { target: { value: dataFutura.toISOString().split("T")[0] } });

        await user.type(screen.getByPlaceholderText(/Insira o título/i), "Relatório Válido");
        await user.type(screen.getByPlaceholderText(/Insira a descrição/i), "Descrição Válida");
        ...
        expect(await screen.findByText(/Data inválida ou fora do limite/i)).toBeInTheDocument();
    ...
    it("deve rejeitar submissão com título contendo espaços ou caracteres inválidos", async () => {
        ...
        const form = screen.getByRole("button", { name: /Adicionar Relatório/i }).closest("form")!;

        await user.type(inputTitulo, "     ");
        fireEvent.submit(form);
        await waitFor(async () => {
            expect(await screen.findByText(/O título não pode ser vazio ou conter caracteres especiais/i)).toBeInTheDocument();
        });
    ...
    it("deve bloquear o upload de arquivos com extensões não autorizadas (.exe, .zip)", async () => {
        ...
        const inputFile = screen.getByTestId("arquivo-input");
        const arquivoInvalido = new File(["virus"], "malicioso.exe", { type: "application/x-msdownload" });
